...
public class ColoredCasingEncasingPonder {
    public static void coloredCasingShaftEncasing(SceneBuilder scene, SceneBuildingUtil util) {
        scene.title("shaft_colored_casing", "Encasing Shafts with colored casings");
        ...
        BlockEntry<EncasedShaftBlock> redEncased = CCBlocks.RED_ENCASED_SHAFT;
        ItemStack redCasingItem = CCBlocks.RED_CASING.asStack();
        ...
        BlockEntry<EncasedShaftBlock> blackEncased = CCBlocks.BLACK_ENCASED_SHAFT;
        ItemStack blackCasingItem = CCBlocks.BLACK_CASING.asStack();

        scene.overlay.showControls(new InputWindowElement(util.vector.topOf(1, 0, 2), Pointing.UP).rightClick()
                .withItem(blackCasingItem), 60);
        scene.idle(7);
        scene.world.setBlocks(black, blackEncased.getDefaultState()
                .setValue(EncasedShaftBlock.AXIS, Axis.X), true);
        scene.world.setKineticSpeed(shaft, 32);
    ...
    public static void coloredCasingCogEncasing(SceneBuilder scene, SceneBuildingUtil util) {
        ...
        BlockEntry<EncasedCogwheelBlock> redEncased = CCBlocks.RED_ENCASED_COGWHEEL;
        ItemStack redCasingItem = CCBlocks.RED_CASING.asStack();

        scene.overlay.showControls(new InputWindowElement(util.vector.topOf(3, 0, 2), Pointing.UP).rightClick()
                .withItem(redCasingItem), 100);
        scene.idle(7);
        scene.world.setBlocks(util.select.position(3, 1, 2), redEncased.getDefaultState()
        ...
        scene.overlay.showControls(new InputWindowElement(util.vector.topOf(2, 1, 2), Pointing.DOWN).rightClick()
                .withItem(redCasingItem), 30);
        scene.idle(7);
        scene.world.setBlocks(small2, redEncased.getDefaultState()
                .setValue(EncasedCogwheelBlock.AXIS, Axis.Y), true);
        scene.world.setKineticSpeed(small2, 32);
        scene.idle(15);

        BlockEntry<EncasedCogwheelBlock> blackEncased = CCBlocks.BLACK_ENCASED_LARGE_COGWHEEL;
        ItemStack blackCasingItem = CCBlocks.BLACK_CASING.asStack();

        scene.overlay.showControls(new InputWindowElement(util.vector.topOf(1, 0, 3), Pointing.UP).rightClick()
                .withItem(blackCasingItem), 60);
        scene.idle(7);
        scene.world.setBlocks(util.select.position(1, 1, 3), blackEncased.getDefaultState()
                .setValue(EncasedCogwheelBlock.AXIS, Axis.Y), true);
        ...
        scene.overlay.showText(70)
                .placeNearTarget()
                .attachKeyFrame()
                .text("Colored Casings can be used to decorate Cogwheels")
